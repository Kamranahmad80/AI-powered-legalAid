import React from 'react';
import styled from 'styled-components';

const SelectorContainer = styled.div`
  flex: 1;
  min-width: 180px;
  box-sizing: border-box;
`;

const Label = styled.label`
  display: block;
  margin-bottom: 4px;
  font-weight: 600;
  color: #333;
  font-size: 14px;
`;

const Select = styled.select`
  width: 100%;
  padding: 8px 30px 8px 12px;
  border-radius: 6px;
  border: 1px solid #ccc;
  background-color: white;
  font-size: 14px;
  color: #333;
  height: 36px;
  appearance: none;
  background-image: url("data:image/svg+xml;charset=US-ASCII,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20width%3D%22292.4%22%20height%3D%22292.4%22%3E%3Cpath%20fill%3D%22%23333%22%20d%3D%22M287%2069.4a17.6%2017.6%200%200%200-13-5.4H18.4c-5%200-9.3%201.8-12.9%205.4A17.6%2017.6%200%200%200%200%2082.2c0%205%201.8%209.3%205.4%2012.9l128%20127.9c3.6%203.6%207.8%205.4%2012.8%205.4s9.2-1.8%2012.8-5.4L287%2095c3.5-3.5%205.4-7.8%205.4-12.8%200-5-1.9-9.2-5.5-12.8z%22%2F%3E%3C%2Fsvg%3E");
  background-repeat: no-repeat;
  background-position: right 8px top 50%;
  background-size: 10px auto;
  
  &:focus {
    border-color: #007bff;
    outline: none;
    box-shadow: 0 0 0 2px rgba(0, 123, 255, 0.25);
  }
`;

const categories = [
  { id: 'housing', label: 'Housing & Tenant Rights' },
  { id: 'employment', label: 'Employment & Labor Law' },
  { id: 'family', label: 'Family Law' },
  { id: 'consumer', label: 'Consumer Rights' },
  { id: 'immigration', label: 'Immigration Law' },
  { id: 'debt', label: 'Debt & Financial Issues' },
  { id: 'healthcare', label: 'Healthcare Law' },
  { id: 'disability', label: 'Disability Rights' },
  { id: 'other', label: 'Other Legal Issues' }
];

const CategorySelector = ({ selectedCategory, onCategoryChange }) => {
  return (
    <SelectorContainer>
      <Label htmlFor="legal-category">Select Legal Category:</Label>
      <Select 
        id="legal-category" 
        value={selectedCategory} 
        onChange={(e) => onCategoryChange(e.target.value)}
      >
        <option value="">-- Select a category --</option>
        {categories.map(category => (
          <option key={category.id} value={category.id}>
            {category.label}
          </option>
        ))}
      </Select>
    </SelectorContainer>
  );
};

export default CategorySelector;
